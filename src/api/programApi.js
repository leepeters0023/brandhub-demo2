...
export const fetchProgramsByTerritory = async (id) => {
  ...
  await axios
    .get(`/api/programs?filter[territory-id]=${id}&filter[is-pre-order]=true&filter[is-pre-order-active]=true`)
    .then((res) => {
...
export const fetchNationalPrograms = async () => {
  const response = { status: "", error: null, data: null };
  await axios
    .get(`/api/programs?filter[type]=National&filter[is-pre-order]=true&filter[is-pre-order-active]=true`)
    .then((res) => {
      let data = dataFormatter.deserialize(res.data);
      response.status = "ok";
      response.data = data;
